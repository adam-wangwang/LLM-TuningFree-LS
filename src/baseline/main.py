...
def train( model, Configs, tokenizer):
    ...
    num_warmup_steps = int(Training_Configs.warmup_ratio * t_total)
    # Prepare optimizer and schedule (linear warmup and decay)d
    no_decay = ["bias", "LayerNorm.weight"]
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)],
    ...
    if os.path.exists(Training_Configs.model_name_or_path):
        ...
        epochs_trained = global_step // (len(train_dataloader) // Training_Configs.gradient_accumulation_steps)
        steps_trained_in_current_epoch = global_step % (len(train_dataloader) // Training_Configs.gradient_accumulation_steps)
...
def load_and_cache_examples(Dataset_Configs, task, tokenizer, split="train"):
    ...
    if os.path.exists(cached_tensors_file) and not Dataset_Configs.overwrite_cache:
        ...
        # modification-241119 指定测试机testset单独的路径，这样做是为了测试数据域不匹配的场景下的性能
        if split == "test":
            examples = get_examples(Dataset_Configs.test_csv_dir)
        else:
...
def main(Configs, seed_shift=0):
    ...
    with open(result_path, "w", encoding="utf-8") as f:
        f.write("test phase:\naccuracy\t{:.4f}\nprecision\t{:.4f}\nrecall\t{:.4f}\nf1_score\t{:.4f}\troc_auc\t{:.4f}"
                .format(test_acc*100,test_precision*100,test_recall*100,test_Fscore*100, roc_auc*100))
